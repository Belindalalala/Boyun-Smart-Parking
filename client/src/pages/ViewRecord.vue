<template>
    <div class="p-4">
      ...
      <div class="flex items-center justify-between mb-4">
        <p class="text-2xl font-bold text-center">车流管理</p>
        <button
        ...
        <table class="table table-auto border-collapse border border-gray-300 w-full">
          ...
          <thead>
            <tr class="bg-gray-200">
              ...
              <th class="px-4 py-2 border">Parking Area</th>
              <th class="px-4 py-2 border">Parking Number</th>
          ...
          <tbody>
            ...
            <tr v-for="user in users" :key="user.id" class="hover:bg-gray-100">
              ...
              <td class="px-4 py-2 border">{{ user.parking_area }}</td>
              <td class="px-4 py-2 border">{{ user.parking_number }}</td>
              <td class="px-4 py-2 border">{{ user.entry_time }}</td>
              <td class="px-4 py-2 border">{{ user.exit_time }}</td>
              <td class="px-4 py-2 border">{{ user.fee }}</td>
  ...
  export default {
    ...
    methods: {
      ...
      updateUser(id) {
        this.$router.push(`/record/update/${id}`);
      },
      deleteUser(id) {
        fetch(`http://localhost:8000/api/parks/${id}`, {
          method: "DELETE",
